    ...
    $.getJSON({
        ...
        success: function (data) { //  contient le HTML renvoyé
            ...
            for (i = 1; i < data.values.length; i++) {
                ...
                html += '<div>';
                html += '<h1 class=" title mt-12 md:mt-0 text-3xl font-semibold text-gray-800  ">' + titre + '</h1>';
                html += '<p class="avis text-base font-medium leading-6 mt-4 text-gray-600   ">' + avis + '</p>';
                html += '</div>';
                html += '<div class="md:mt-0 mt-8">';
                html += '<p class="nom text-lg font-medium leading-4 text-gray-800  ">' + nom + '<span> ' + prenom + '</span></p>';
                html += '<p class="status text-base leading-4 mt-2 mb-4 text-gray-600   ">' + statut + '</p>';
                html += '</div>';
                html += '</div>';
                html += '</div>';
                html += '</div>';
            ...
            TweenLite.fromTo(".title", 0.4, fromProperties, toProperties);
            TweenLite.fromTo(".avis", 0.6, fromProperties, toProperties);
            TweenLite.fromTo(".nom", 0.8, fromProperties, toProperties);
            TweenLite.fromTo(".status", 1, fromProperties, toProperties);
